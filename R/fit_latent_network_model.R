...
fit_latent_network_model = function(data=model_dat,
                                    fpr_regression,
                                    rtt_regression,
                                    theta_regression,
                                    focal_regression,
    ...
    for(i in 1:ncol(dyad_model_matrix)){
     ...
     n_dyadic_params = ncol(dyad_model_matrix)
      } else{
        n_dyadic_params = 1
        dyad_dims = c(data$N_id, data$N_id, n_dyadic_params)
        dyad_dat_out = array(NA, c(dyad_dims[1], dyad_dims[2], n_dyadic_params))
        dyad_dat_out[,,1] = matrix(rep(1, dyad_dims[1]*dyad_dims[2]), nrow=dyad_dims[1], ncol=dyad_dims[2])
    ...
    data$N_params = c(ncol(data$focal_set), ncol(data$target_set), ncol(data$fpr_set), ncol(data$rtt_set), ncol(data$theta_set), n_dyadic_params)

    data$export_network = ifelse(return_latent_network==TRUE, 1, 0)

    ############################################################################# Fit model
    
    model = cmdstan_model(paste0(path.package("STRAND"),"/","latent_network_model.stan"))
