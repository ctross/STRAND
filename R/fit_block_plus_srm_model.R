...
#' @param mode A string giving the mode that stan should use to fit the model. "mcmc" via Stan is default and recommended, "numpyro" is substantially faster if you have a Python pipeline. STRAND has functions to make processing the mcmc samples easier. Other options are "optim", to
...
fit_block_plus_social_relations_model = function(data,
    ...
    if(mode=="numpyro"){
      ...
      fit = fit_bsrm_with_numpyro(data, mcmc_parameters)
    }
