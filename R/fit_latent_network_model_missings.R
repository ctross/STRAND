...
fit_latent_network_model_missings = function(data,
    ...
    ################################################################ Dyad model matrix
     ...
     if(data$N_individual_predictors>0){
      data$fpr_set = model_matrix_strand(fpr_regression, model.frame(~ ., data$individual_predictors, na.action=na.pass), "fpr_regression")
      data$rtt_set = model_matrix_strand(rtt_regression , model.frame(~ ., data$individual_predictors, na.action=na.pass), "rtt_regression")
      data$theta_set = model_matrix_strand(theta_regression , model.frame(~ ., data$individual_predictors, na.action=na.pass), "theta_regression")
