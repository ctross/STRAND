...
simulate_srm_network_lv = function(N_id = 99,                            # Number of respondents
                                    B = -4,                           # Tie log odds
...
# Simulate outcomes
 y_true[i,j] = ifelse( sr[i,1] + sr[j,2] + dr[i,j] > 0, 1, 0)
 y_true[j,i] = ifelse( sr[j,1] + sr[i,2] + dr[j,i] > 0, 1, 0)
