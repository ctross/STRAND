...
#'
#' @param results The results object from summarize_strand_results.
#' @param submodels Which submodels to plot? Supported: "False positive rate", "Recall of true ties", "Theta: question-order effects", "Focal effects: Out-degree", "Target effects: In-degree", "Dyadic effects", "Other estimates"
#' @param normalized Should effects be normalized? Do not use for correlations and variance terms.
#' @param only_slopes If TRUE then technical parameters are omited, and only slopes and intercepts are plotted.
#' @param only_technicals If TRUE then technical parameters are plotted, and slopes and intercepts are omited
#' @param site A string to indicate group or feildsite.
#' @param export_as_table If TRUE, the data are not plotted, and a data.frame is returned. This is useful for making complex ggplot objects with multiple groups/sites.
...
strand_caterpillar_plot = function(results, submodels=NULL, normalized=FALSE, only_slopes=FALSE, only_technicals=FALSE, site="BOB", export_as_table=FALSE){
  ...
  for(k in 1:length(results$summary_list)){
   ...
   colnames(dat[[k]]) = c("Variable", "Median", "LI", "HI", "Mean","SD","P", "SubModel")
   for(j in 2:6)
...
colnames(df) = c("Variable", "Median", "LI", "HI", "Mean","SD", "P", "SubModel")
