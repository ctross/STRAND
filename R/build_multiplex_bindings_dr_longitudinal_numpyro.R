...
build_multiplex_bindings_dr_longitudinal_numpyro = function(data, verbose = FALSE){
  ...
  out_full_bindings = do.call(rbind,out_list)
  in_full_bindings = do.call(rbind,in_list)

  # Build dr bindings 2
  out_full_bindings2 = out_full_bindings
  out_full_bindings2[,2] = out_full_bindings2[,2]+K

  in_full_bindings2 = in_full_bindings
  in_full_bindings2[,2] = in_full_bindings2[,2]+K
 
  out_dr_bindings = cbind(rep(1,K-1), rep(1+K,K-1))
  in_dr_bindings = cbind(c(2:K),c(2:K)+K)

  out_all = rbind(out_full_bindings, out_full_bindings2, out_dr_bindings)
  in_all = rbind(in_full_bindings, in_full_bindings2, in_dr_bindings)
  ...
  data$numpyro_dr_long_bindings_out = out_all
  data$numpyro_dr_long_bindings_in = in_all
   return(data)
}
