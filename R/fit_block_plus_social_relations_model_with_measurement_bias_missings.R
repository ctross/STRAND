#' A function to run combined stochastic block and social relations model with measurement error using the STRAND framework
#' 
...
#' @param sampling_regression A formula for the predictors of sampling (i.e., exposure time). This should be specified as in lm(), e.g.: ~ Age * Education. If not of interest write: ~ 1.
#' @param censoring_regression A formula for the predictors of censoring (i.e., lack of detectability). This should be specified as in lm(), e.g.: ~ Age * Education.
...
#' @examples
#' \dontrun{
#' fit = fit_block_plus_social_relations_model_with_measurement_bias_missings(data=model_dat,
...
fit_block_plus_social_relations_model_with_measurement_bias_missings = function(data,
                                    block_regression,
                                    focal_regression,
                                    target_regression,
                                    dyad_regression,
    ...
    model = cmdstanr::cmdstan_model(paste0(path.package("STRAND"),"/","block_plus_social_relations_model_with_measurement_bias_missings.stan"))
    ...
    attr(bob, "model_type") = "SRM+SBM+ME"
