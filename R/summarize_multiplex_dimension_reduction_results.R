...
#'
#' @param input A STRAND model object, obtained by fitting a combined stochastic block and social relations model for multiplex dimension reduction.
...
#' @examples
#' \dontrun{
#' res = summarize_multiplex_dimension_reduction_results(input = fit)
#' }
#'

summarize_multiplex_dimension_reduction_results = function(input, include_samples=TRUE, HPDI=0.9){
    ...
    ################### SRM model
     ...
     results_srm_loadings = matrix(NA, nrow=(input$data$N_responses*2) , ncol=7)

    ######### Calculate all loadings
    for(m in 1:input$data$N_responses){
     results_srm_loadings[m,] = sum_stats(paste0("base-rate, ", attr(input$data,"layer_names")[m] ), samples$srm_model_samples$loadings[,m,1], HPDI)
    }

    for(m in 1:input$data$N_responses){
     results_srm_loadings[m + input$data$N_responses,] = sum_stats(paste0("loading strength, ", attr(input$data,"layer_names")[m] ), samples$srm_model_samples$loadings[,m,2], HPDI)
    }

     results_list[[1]] = results_srm_loadings
   ...
   ############# Finally, merge all effects into a list
     ...
     names(results_list) = c("Layer loadings", "Focal effects: Out-degree", "Target effects: In-degree", "Dyadic effects", "Other estimates")
          
   results_out = rbind(results_srm_loadings, results_srm_focal, results_srm_target, results_srm_dyadic, results_srm_base)
