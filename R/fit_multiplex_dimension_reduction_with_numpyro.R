#' A NumPyro implementation of the STRAND latent-network SRM with dimension reducation
#'
...
#' @examples
#' \dontrun{
#' res = fit_bsrm_with_numpyro(data, mcmc_parameters)
#' }
#'

fit_multiplex_dimension_reduction_with_numpyro = function(
    data,
...
# Import numpy, jax, numpyro, and numpyro.distributions
  ...
  numpyro_multiplex_dimension_reduction = NULL
  reticulate::source_python(paste0(path.package("STRAND"),"/","numpyro_multiplex_dimension_reduction.py"))        
...
########################### NumPyro backend prep
  ...
  prior_19_1 = as.numeric(priors_np[19L,1L])

  prior_23_1 = as.numeric(priors_np[23L,1L])
  prior_23_2 = as.numeric(priors_np[23L,2L])

  S = np$array(as.numeric(data$expected_ties))
  penalty = np$array(as.numeric(S^(1/prior_19_1)))

  outcome_mode = as.integer(data$outcome_mode)
  export_network = as.integer(data$export_network)
  stop_reflection_invariance = as.integer(data$stop_reflection_invariance)
  ...
  kernel = NUTS(numpyro_multiplex_dimension_reduction,
  ...
  mcmc$run(mcmc_key, np, jax, numpyro, dist, jnp,
           ...
           jnp$array(S), jnp$array(penalty), stop_reflection_invariance,
           jnp$array(prior_23_1), jnp$array(prior_23_2),
           jnp$array(long_focal_set_np), jnp$array(long_target_set_np), 
           jnp$array(long_dyad_set_np), jnp$array(long_block_set_np),  
