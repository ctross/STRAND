functions{
    ...
    //# CES function 2
    vector CES_1(vector K, vector L, real alpha, real sigma){
      real rho = (sigma - 1)/sigma;
      return(pow(alpha*pow(K, rho) + (1-alpha)*pow(L, rho), (1/rho)));
    } 

    //# CES function 3
    vector CES_inf_1(vector K, vector L, real alpha){
      return(alpha*K + (1-alpha)*L);
    } 
...
parameters{
    ...
    array[N_id, N_id, N_responses] real<lower=0, upper=1> A;   //# Network of ties for each timepoint
}
      ...
      //################################################################ Model of social learning
        for(j in 1:N_id){
          ...
          psi[j] = inv_logit(sum(br) + 
           ...
           if(i != j){ 
           outcomes[i,j,t] ~ binomial(exposure[i,j,t], A[i,j,t]);
            }
          }
