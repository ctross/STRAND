def numpyro_longitudinal(
    ...
    long_block_set_np,
    N_dyads, N_id,
    N_var_focal, N_var_target, N_var_dyad, N_var_block, N_layers,
    numpyro_dr_bindings_out_1, numpyro_dr_bindings_out_2, 
    numpyro_dr_bindings_in_1, numpyro_dr_bindings_in_2, 
    numpyro_dr_long_bindings_out_1, numpyro_dr_long_bindings_out_2,
    numpyro_dr_long_bindings_in_1, numpyro_dr_long_bindings_in_2,
    numpyro_gr_long_bindings_out_1, numpyro_gr_long_bindings_out_2,
    numpyro_gr_long_bindings_in_1, numpyro_gr_long_bindings_in_2,
    A_F, A_T, A_D):
    ...
    if coefficient_mode == 1:
        ...
        block_effects = numpyro.sample("block_effects", dist.Normal(loc=block_mu_np, scale=block_sigma_np))

    elif coefficient_mode == 2:
        focal_effects = jnp.tile(numpyro.sample("focal_effects", dist.Normal(loc=jnp.full((1, N_var_focal), prior_12_1), scale=jnp.full((1, N_var_focal), prior_12_2))), (N_layers, 1))
        target_effects = jnp.tile(numpyro.sample("target_effects", dist.Normal(loc=jnp.full((1, N_var_target), prior_13_1), scale=jnp.full((1, N_var_target), prior_13_2))), (N_layers, 1))
        dyad_effects = jnp.tile(numpyro.sample("dyad_effects", dist.Normal(loc=jnp.full((1, N_var_dyad), prior_14_1), scale=jnp.full((1, N_var_dyad), prior_14_2))), (N_layers, 1))
        block_effects = jnp.tile(numpyro.sample("block_effects", dist.Normal(loc=mean_block_mu_np, scale=mean_block_sigma_np)), (N_layers, 1))

    # Linear predictor for each observation        
    mu = (
        jnp.einsum("ldxf,lf->ldx", long_focal_set_np, focal_effects * A_F) +
        jnp.einsum("ldxf,lf->ldx", long_target_set_np, target_effects * A_T) +
        jnp.einsum("ldxf,lf->ldx", long_dyad_set_np, dyad_effects * A_D) +
        jnp.einsum("ldxf,lf->ldx", long_block_set_np, block_effects)
    )

    # Dyad random effects
    if export_network == 1:
        dr_raw = numpyro.sample("dr_raw", dist.Normal(jnp.zeros((N_layers * 2, N_dyads)), 1))
    else:
        dr_raw = numpyro.sample("dr_raw", dist.Normal(jnp.zeros((N_layers * 2, N_dyads)), 1), infer={"num_samples": 0})
    
    dr_L = numpyro.sample("dr_L", dist.LKJCholesky(2 * N_layers, prior_18_1))
    dr_sigma = numpyro.sample("dr_sigma", dist.TruncatedNormal(loc=jnp.full((N_layers,), prior_16_1), scale=jnp.full((N_layers,), prior_16_2), low=0.0))
    dr_sigma_temp = jnp.expand_dims(jnp.repeat(dr_sigma, 2), 1)*dr_L
    dr = jnp.matmul(dr_sigma_temp, dr_raw)
    dr_long = jnp.stack(jnp.split(dr, 2), axis=2)

    # Sender and receiver effects
    sr_raw = numpyro.sample("sr_raw", dist.Normal(jnp.zeros((N_layers * 2, N_id)), 1))
    sr_L = numpyro.sample("sr_L", dist.LKJCholesky(N_layers * 2, prior_17_1))
    sr_sigma = numpyro.sample("sr_sigma", dist.TruncatedNormal(loc=jnp.full((N_layers * 2,), prior_15_1), scale=jnp.full((N_layers * 2,), prior_15_2), low=0.0))
    sr_sigma_temp = jnp.expand_dims(sr_sigma, 1)*sr_L
    gr = jnp.matmul(sr_sigma_temp, sr_raw)
    gr_sender, gr_receiver = jnp.split(gr, 2)
