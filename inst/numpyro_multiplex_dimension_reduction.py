def numpyro_multiplex_dimension_reduction(
    ...
    S, penalty, stop_reflection_invariance,
    ...
    # Outcome regression
    alpha = numpyro.sample("alpha", dist.Normal(jnp.zeros((N_layers, 1, 1)), 5.0))
    beta_raw = numpyro.sample("beta_raw", dist.Normal(jnp.zeros((N_layers - 1, 1, 1)), 5.0))
    ...
    if stop_reflection_invariance == 1:
        numpyro.factor("reflection_penalty", -0.5 * (jnp.sum(p) - S)**2 / penalty**2)
