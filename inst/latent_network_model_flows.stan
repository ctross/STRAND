functions{
    ...
    real prob_sgij( int[] sij, int[] sji, int[] gij, int tie, vector fpri, vector fprj, vector rtti, vector rttj, real theta, vector decay_curve, vector flow_rate) {
       vector[2] y; 
       vector[size(gij)] z;          
    ...
    if(tie==0){
        ...
        for (k in 1:size(gij)){
        z[k] = bernoulli_logit_lpmf(gij[k] | fpri[3]);
                }
        ...
        for (k in 1:size(gij)){
            if(gij[k]==1){
                 z[k] = log(flow_rate[k]) + log_inv_logit(rtti[3]);
                } else{
                 z[k] = log_sum_exp(log1m(flow_rate[k]), log(flow_rate[k]) + log1m_inv_logit(rtti[3]));
                  }
                }


        //# prob i says i helps j
        y[1] = bernoulli_logit_lpmf(sij[1] | rtti[1] + dot_product(decay_curve, to_vector(gij)));

        //# prob j says i helps j
        if(sji[1] == 0){
        y[2] = bernoulli_logit_lpmf(sji[2] | rttj[2] + dot_product(decay_curve, to_vector(gij)));
                      }
              else{
        y[2] = log_mix(theta, 
                      bernoulli_logit_lpmf(sji[2] | 1),
                      bernoulli_logit_lpmf(sji[2] | rttj[2] + dot_product(decay_curve, to_vector(gij)))
                      ); 
        }
      }
        return(sum(y) + sum(z));
...
parameters{
    ...
    vector<lower=0, upper=1>[N_periods] flow_rate;

    //# Effects of true flows on responses
    real<lower=0> effect_max;
    real<lower=0> effect_decay;
...
model{
  ...
  matrix[N_id, N_id] mixed_p;
    ...
    effect_max ~ gamma(priors[20,1], priors[20,2]);
    effect_decay ~ exponential(priors[21,1]);

    flow_rate ~ beta(priors[22,1], priors[22,2]);
    ...
    for ( i in 1:N_id ) {
        for ( j in 1:N_id ) {
            if ( i != j ) {
                ...
                for(tie in 0:1) {
                  terms[tie+1] = prob_sgij(outcomes[i,j,], outcomes[j,i,], flows[i,j,], tie, fpr[i], fpr[j], rtt[i], rtt[j], theta[j], decay_curve, flow_rate);                                   
                }
