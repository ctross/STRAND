...
**STRAND** is an R package designed for simulation and analysis of network data.  The package can be used to simulate network data under stochastic block models, social relations models, and latent network models. These tools allow for simulation not only of realistic human---or animal---social networks, but also allow researchers to simulate the effects of potential biases—like respondents falsely reporting ties or failing to recall real ties—on network level properties. 
...
################################### Install and/or load
 ...
 library(STRAND)
```
WARNING: THIS PACKAGE IS STILL IN DEVELOPMENT AND MAY BE UNSTABLE. WE ARE CURRENTLY UNDER PEER REVIEW. THE CURRENT PACKAGE IS A BETA VERSION ONLY.
