###############################################################################################
#
#   Longitudinal analyses with JAX 
#
###############################################################################################
 library(stringr)
 library(ggplot2)
...
# Base settings
 N_id = 80       # Individuals in network
...
#################################################### Fit Models
fit_numpyro = fit_longitudinal_model(long_data=long_dat,
                               ...
                               mcmc_parameters = list(seed = 67, chains = 1, parallel_chains = 1, refresh = 1, iter_warmup = 1000,
                                iter_sampling = 1000, max_treedepth = 11, adapt_delta = 0.95, cores=4, chain_method="vectorized"),
                               priors=NULL
                               )
...
fit_stan = fit_longitudinal_model(long_data=long_dat,
                               block_regression = ~ Coloration + Sex,
                               focal_regression = ~ Age + Food + Mass + Size + Strength,
                               target_regression = ~ Age + Food + Mass + Size + Strength,
                               dyad_regression = ~ Agression + Distance + Dominance + Friendship,
                               ...
                               random_effects_mode="fixed",
                               mode="mcmc",
                               mcmc_parameters = list(seed = 11, chains = 1, parallel_chains = 1, refresh = 1, iter_warmup = 1000,
...
########################################################### Coeff recovery
######### NumPyro
out = longitudinal_plot(fit_numpyro, type="coefficient", 
    parameter_set = list(
...
######### Stan
out = longitudinal_plot(fit_stan, type="coefficient", 
    parameter_set = list(
    focal="Mass", focal="Strength", focal="Age", focal="Food", focal="Size",
    target="Mass", target="Strength", target="Age", target="Food", target="Size",
    dyadic="Agression", dyadic="Distance", dyadic="Dominance", dyadic="Friendship"),
 ...
 ################# Plot
 pal = plvs_vltra("mystic_mausoleum",elements=c(2,4))

 out3all = rbind(out3_np, out3_stan)

 p0 = ggplot(out3all, aes(x=extra_short_names, y=as.numeric(Median), ymin=as.numeric(l), ymax=as.numeric(h), group=Method, color=Method))+ 
     geom_linerange(size=1, position = position_dodge(width = 0.3)) + facet_grid(type~time_point, scales="free", space="free") +
     geom_point(size=2, position = position_dodge(width = 0.3))+ 
     geom_point(aes(x=extra_short_names, y=as.numeric(True)),size=2, position = position_dodge(width = 0.3),color="deeppink3",shape=18)+
     geom_hline(aes(yintercept=0),color="black",linetype="dashed")+
     labs(y="Effect size", x="Variable") + 
...
########################################################### Dyadic recovery
 out = longitudinal_plot(fit_numpyro, type="dyadic", 
    palette=pal,
    normalized=FALSE,
    plot = FALSE,
    export_as_table=TRUE
 ...
 out2 = longitudinal_plot(fit_stan, type="dyadic", 
    palette=pal,
    normalized=FALSE,
    plot = FALSE,
    export_as_table=TRUE
  ...
  out2$True = dr_Rho[1,2:10]
  out2$Method = "Stan"

  out3all = rbind(out, out2)

 pal = plvs_vltra("mystic_mausoleum", elements=c(1,9,2))

 out3all$Method2 = paste(out3all$Method, out3all$rs_type)

 p1 = ggplot(out3all, aes(x=Time2, y=as.numeric(rs_m), ymin=as.numeric(l), ymax=as.numeric(h), group=Method2, color=rs_type, linetype=Method))+ 
     geom_linerange(size=1, position = position_dodge(width = 0.3)) +
     geom_point(size=2, position = position_dodge(width = 0.3))+ geom_point(aes(x=Time2, y=as.numeric(True)),size=3, position = position_dodge(width = 0.3),color="deeppink3",shape=18)+
     geom_hline(aes(yintercept=0),color="black",linetype="dashed")+
...
out = longitudinal_plot(fit_numpyro, type="generalized", 
    palette=pal,
    normalized=FALSE,
    plot = FALSE,
    export_as_table=TRUE
 ...
 out2 = longitudinal_plot(fit_stan, type="generalized", 
    palette=pal,
    normalized=FALSE,
    plot = FALSE,
    export_as_table=TRUE
 ...
 out2$Method = "Stan"
 out2$True = c(sr_Rho[1,2:5], sr_Rho[6,7:10], sr_Rho[1,7:10], sr_Rho[5,9:6], sr_Rho[1,6])

 pal = plvs_vltra("mystic_mausoleum", elements=c(1,2,9,3,4))

 out3all = rbind(out, out2)

 out3all$Method2 = paste(out3all$Method, out3all$Type)

 p2 = ggplot(out3all, aes(x=Time, y=as.numeric(rs_m), ymin=as.numeric(l), ymax=as.numeric(h), group=Method2, color=Type, linetype=Method))+ 
     geom_linerange(size=1, position = position_dodge(width = 0.3)) +
     geom_point(size=2, position = position_dodge(width = 0.3))+ geom_point(aes(x=Time, y=as.numeric(True)),size=3, position = position_dodge(width = 0.3),color="deeppink3",shape=18)+
     geom_hline(aes(yintercept=0),color="black",linetype="dashed")+
     labs(y="Effect size", x="Time step") + 
     theme(strip.text.x = element_text(size=12,face="bold"), 
      strip.text.y = element_text(size=12,face="bold"),
...
res_blocks_np = res_blocks
res_blocks_np$Method = "NumPyro"
...
res_blocks = rbind(res_blocks_1, res_blocks_2)
res_blocks_stan = res_blocks
res_blocks_stan$Method = "Stan"

########################## Plot
res_blocks = rbind(res_blocks_np, res_blocks_stan)

pal = plvs_vltra("mystic_mausoleum",elements=c(2,4))

p3 = ggplot(res_blocks, aes(x = Variable, y = M, ymin = L, ymax = H, color=Method, group=Method)) + 
           geom_linerange(size = 1, position = position_dodge(width = 0.3)) + 
           geom_point(size = 2, position = position_dodge(width = 0.3)) +
           geom_point(size = 3, position = position_dodge(width = 0.3), aes(x = Variable, y = True), color="deeppink3", shape=18) +
           ...
           theme(legend.position="bottom")  + scale_color_manual(values = pal) 
p3


########################################
multiplex_plot(fit_numpyro, type="dyadic", HPDI=0.9, plot = TRUE)
multiplex_plot(fit_numpyro, type="generalized", HPDI=0.9, plot = TRUE)

multiplex_plot(fit_stan, type="dyadic", HPDI=0.9, plot = TRUE)
multiplex_plot(fit_stan, type="generalized", HPDI=0.9, plot = TRUE)
